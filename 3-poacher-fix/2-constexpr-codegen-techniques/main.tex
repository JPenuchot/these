...
from non-literal data structures generated with \constexpr functions.
...
The solutions evaluated in this paper are still workarounds.
...
pointer tree returned by a \constexpr function into code.
...
a pointer tree generated from a \constexpr function as a template parameter
for code generation.
...
Its result can be used to produce literal \constexpr results,
...
In listing \ref{lst:pass-by-generator-example}, we show how a \constexpr
...
related to \constexpr memory constraints and compiler or library support.
...
\begin{lstlisting}[
  ...
  caption=\constexpr tree serialization implementation
]{}
...
the functions being \constexpr.

Once the data is serialized, it can be converted into a static array container.
This can be done because the generator function is \constexpr, therefore it
can be used to produce \constexpr values. The size of the resulting
...
function that takes a \constexpr tree generator function as an input,
...
\constexpr allocated memory either.
...
\item
Code generation from \constexpr dynamic structures is not a trivial process.
It still requires advanced \cpp knowledge to understand the limits of what
is or isn't possible to do with \nttps and \constexpr functions.

While the examples show working examples for code generation from \constexpr
function results with \constexpr allocated memory, they do not show the time
it takes to comply with \cpp \constexpr restrictions.

% TODO: Phrasing
For example, condition results in \lstinline{if constexpr} statements must be
stored in a \constexpr variable if the result is evaluated from an temporary
object that contains \constexpr allocated memory.

\item
The easiest way to get around \nttp constraints, \ie the pass-by-generator
...
\item
Compiler support for \constexpr programming and constant evaluation in general
...
\constexpr functions simply because the \cpp standard lacks \constexpr
qualfications for their methods (and \constexpr qualification is not implicit).
...
that are usable in \constexpr functions in \cpp{}20, which is enough to make
the previous examples run on Clang in \cpp{}20.

\end{itemize}

So far we can conclude that while being doable, code generation from \constexpr
...
from \constexpr allocated memory to non-type template parameters.
