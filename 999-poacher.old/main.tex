...
Finally another use of Cest is to make more \constexpr functions
\cpp20 compatible. For example, \lstinline{cest::unique_ptr} is compatible
with \cpp20, even though \lstinline{std::unique_ptr} was made \constexpr
compatible in \cpp23. When used in \cpp23 mode, some containers are replaced
with aliases of the STL's containers. This ensures that the best implementation
available is always the one being used, and avoids long term maintainance
burdens in the as Cest is mostly meant to be a transitory container library.
\\

I contributed to the Cest project by implementing its \constexpr compatible
alternative to \lstinline{std::unique_ptr}, which was essential for the initial
