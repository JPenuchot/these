...
By definition, a \acrfull{dsl} is a computer language
...
features for a particular domain. \acrfull{dsel}\acrpluralsuffix{}
are a subclass of \acrshort{dsl} that rely on an existing general-purpose language to host
it. \acrshort{dsel}\acrpluralsuffix{} then reuse the host language syntax and tool ecosystem to be compiled
or interpreted.
...
and not all constant expression results can be used as \acrshort{nttp}\acrpluralsuffix{}.
...
\item Case \lstinline{c}: similarly to case \lstinline{a},
      \lstinline{generate}'s return value cannot be used as an \acrshort{nttp} because it
      is not a literal value.
\item Case \lstinline{d}: function references are allowed as \acrshort{nttp}\acrpluralsuffix{}.
\end{itemize}
...
content cannot be transferred to \acrshort{nttp} compatible data structures.
...
directly as an \acrshort{nttp} for code generation.
...
% that return abstract syntax trees using \gls{constexpr} allocated memory through
...
\acrshort{dsel}\acrpluralsuffix{} in \cpp use template metaprogramming via the \textit{Expression
Template} idiom.
...
the use of \gls{constexpr} strings to embed arbitray \acrshort{dsel}\acrpluralsuffix{}. One major example is
