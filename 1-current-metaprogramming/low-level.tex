...
versions of those routines emerged. In this section, we propose
...
In this section, we describe how we obtained optimized generated
\cpp codes for the gemv routine to make it reach its
peak performance on different target CPU architectures (Intel
x86 and ARM in this section). Our gemv \cpp generated kernels
...
This chapter is organized as follows: in Section II, we describe
...
\gls{simd} layer and a set of code generation techniques stemmed
...
this section. Such systems includes:
...
architectures including \gls{simd} capable CPU.

\item
BLIS\cite{hpcs14}:
a framework for generating BLAS like operations
in ISO C99 from a small subset of kernels that can
...
In this section, we will focus on language-based metaprogramming
...
Classical design of metaprograms in \cpp usually relies
...
usually high. Template metaprograms were complex to write
...
\subsection{
  Application to \acrlong{hpc} code generation
}
...
through this section and the example of the level 2 BLAS
